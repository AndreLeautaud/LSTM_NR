...
M=readtable('C:\n2o_nn\LSTM_June2022\GlobalN2ODB_NN_cleaned_dataset.csv'); %load this csv file(around 6 mb) into latlab drive and import it from there into the variable M
...
for i = 1:size(n2o,1)
    ...
    for j = 1:size(n2o,2)
        ...
        split = isnan(n2oval)||isnan(rainval)||isnan(wfpsval); 
        if ~split
...
for i = 1:length(d) % for every entry of d
    ...
    while count < (length(seq)/k*aug) % this condition is met when we have enough subsequences to satisfy aug
        ...
        XTrain1 = [XTrain1;[splitseq(1,:).*sampidxn2o;rainseq;wfpsseq;sampidxn2o]]; 
...
%for i=1:length(Weights)
    %weights2 = [weights2;Weights(i,:)/s];%Weights(i)=Weights(i)/s;
...
for i = 1:length(dvalidation)
    ...
    while count < (length(seq)/k*aug)
        ...
        % 
        XValidation1 = [XValidation1;[splitseq(1,:).*sampidxn2o;rainseq;wfpsseq;sampidxn2o]];
...
W_fmt = 'Weights_%s_SL%0.0f_n2osf%02d.mat';
...
W_fname = sprintf(W_fmt,covariates,SL,sampdays);
